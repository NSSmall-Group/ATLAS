...
def create_graphtool_graph(source_file):
    ...
    with open(source_file, "r") as file:
        ...
        G.save_graph(source_file.replace("output/",'vis')+'.html')
...
network.on("afterInit", function() {
    ...
    window.initColorData();
        ...
        html_content = ''
        vis_js=''
        with open("vis-network.min.js", "r", encoding="utf-8") as f:
            vis_js = f.read()
        with open('visseq_graph_testing_preprocessed_logs_S2-CVE-2015-3105_windows.dot.txt.html', "r", encoding="utf-8") as f:
            html_content = f.read()
        html_content = html_content.replace(
            ...
            f'<script type="text/javascript">{vis_js}</script>'
        )
        with open("vis_offline.html", "w", encoding="utf-8") as f:
