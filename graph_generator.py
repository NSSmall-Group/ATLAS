...
def quote_colon_labels(G):
    G_new = nx.DiGraph() if G.is_directed() else nx.Graph()
    # 处理节点
    for n, attrs in G.nodes(data=True):
        new_n = f'"{n}"' if ':' in str(n) and not str(n).startswith('"') else n
        new_attrs = {
            k: (f'"{v}"' if isinstance(v, str) and ':' in v and not v.startswith('"') else v)
            for k, v in attrs.items()
        }
        G_new.add_node(new_n, **new_attrs)
    # 处理边
    for u, v, attrs in G.edges(data=True):
        new_u = f'"{u}"' if ':' in str(u) and not str(u).startswith('"') else u
        new_v = f'"{v}"' if ':' in str(v) and not str(v).startswith('"') else v
        new_attrs = {
            k: (f'"{v}"' if isinstance(v, str) and ':' in v and not v.startswith('"') else v)
            for k, v in attrs.items()
        }
        G_new.add_edge(new_u, new_v, **new_attrs)
...
if __name__ == '__main__':
	...
	for file in os.listdir("output"):
		if file.startswith("testing_preprocessed_logs"):
			#if "_h1" in file:
			#	continue
			...
			#construct_G(False) # no executed edges
			#n_timestamp = G.nodes(data=True)[user_node]['timestamp']
			#
			#construct_G_subgraph() # find our subgraph with no executed edges
			#G_subgraph_nodes_size = str(len(G_subgraph.nodes()))
			#G_subgraph_edges_size = str(len(G_subgraph.edges()))
			#
			#find_G_subgraph_potential_roots() # filling found_roots[]
			...
			#	print n
			'''
			explore_roots() # construct ROOT_G_subgraph
			if ROOT_G_subgraph == None:
				ROOT_G_subgraph = G.subgraph(G_subgraph.nodes()).copy()
			...
			new_G = quote_colon_labels(ROOT_G_subgraph)
			...
			nx.nx_pydot.write_dot(new_G, "output/graph_" + file + ".dot")
