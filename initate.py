...
tensorflow.random.set_seed(seed)  # 设置 TensorFlow 随机种子
...
# 修改所有Keras导入路径（TensorFlow 2.12下直接使用tf.keras即可）
from tensorflow.keras.models import Sequential, Model, model_from_json, load_model
from tensorflow.keras.layers import (
    Input, Dense, Dropout, Flatten, Embedding,
    LSTM, Bidirectional, GRU, Conv1D,
...
if DO_TRAINING:
    ...
    # model = load_model('output/ori_model.h5', compile=False)


# converter = tf.experimental.tensorrt.Converter(input_saved_model_dir="output/model.h5")
# converter.convert()
...
def load_malicious_labels(file):
    global mlabels_file, mlabels_file_events, malicious_labels, malicious_labels_events, start_time
    start_time = tm.time()
...
def testing_suggest_ground_truth(lines, possible_labels):
    ...
    global prediction_counter, classified_words, classified_words_prediction, classified_words_proba, start_time
    ...
    while True:
        ...
        if prediction_counter >= maximum_number_of_test_iterations:  # 1
            ...
            with open(w_current_file, 'w') as f:
                ...
                file = open("sys", 'a')
                file.write(str(time) + " atlas.py excuted\ntime cost:{}s\nmemory peek:{}MB\n".format(time_cost,
                                                                                                     peak / 1024 / 1024))
                exit()
        ...
        # print(x_test)
        # print(z_test)
        with open('output/x_test.pkl', 'wb') as file:
            pickle.dump(x_test, file)
        with open('output/z_test.pkl', 'wb') as file:
            pickle.dump(z_test, file)
        with open('output/user_artifact.txt','w') as file:
            file.write(user_artifact)
        exit()
...
if __name__ == '__main__':
    ...
    if DO_TRAINING:
        ...
        converter = tensorflow.experimental.tensorrt.Converter(input_saved_model_dir="output/saved_model")
        converter.convert()
        converter.save("output/model_trt")
        print("TensorRT 模型已保存！")
        current, peak = tracemalloc.get_traced_memory()
        file = open("sys", 'a')
        file.write(str(tm.time()) + " atlas.py excuted\ntime:{}s\n memory:{}MB\n".format(done-start_time, peak / 1024 / 1024))
        exit()
    ...
    # testing
    for file in os.listdir("output"):
        if file.startswith("seq_graph_testing_") and not file.endswith('.dot') and not file.endswith('.gexf'):
            load_malicious_labels(file)
