...
def generate_model():
	...
	model.add(Conv1D(filters, kernel_size))  # 64
...
if DO_TRAINING:
	...
	saved_model_dir = 'output/trt_model'  # 指定已转换的TF-TRT模型路径
...
def predict_labels():
	...
	if CUSTOM_FIT == 0:
		...
		print(infer)
		# 使用 x_test 进行推理（假设 x_test 是一个符合模型输入的 Tensor
		x_test_tensor = tf.convert_to_tensor(x_test, dtype=tf.float32)
		print(x_test_tensor.device)
		#print(infer.structured_outputs)
		#proba_output = infer(embedding_1_input=x_test_tensor)['dense_1']
		batch_size = 256 # 每批处理的样本数量
		num_samples = x_test_tensor.shape[0]  # 样本数量，假设是 800
		batch_steps = num_samples // batch_size + (1 if num_samples % batch_size != 0 else 0)  # 计算批次数
		load_time = tm.time()
		...
		for i in range(batch_steps):
			...
			x_batch = x_test_tensor[start:end]  # 获取当前批次的数据

			# 执行推理
			batch_output = infer(embedding_1_input=x_batch)
			proba_output.append(batch_output['dense_1'])
			
		f_time = tm.time()
		print(f_time-load_time)
		print(":##############################")
		# 拼接所有批次的输出，得到最终的预测结果
		proba_output = tf.concat(proba_output, axis=0)


		# 根据阈值进行二分类预测
		prediction = (proba_output > 0.4).numpy().astype(int).flatten()
		prediction_proba = proba_output.numpy().flatten()
